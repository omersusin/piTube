...
- **Account library sync** — "Sync now" fully crawls your liked videos (continuation-token pagination, no more 100-item cap), playlists and subscriptions from the WEB client, plus a silent daily **auto-sync**. The subscriptions crawl pages through **every** page of FEchannels via continuation tokens, with transient-error retry and a repeated-token guard, so a sync never stops on a partial list
- **Subscription transfer** — import and export your follow list from Settings as **NewPipe JSON**, a **Google Takeout CSV**, or **OPML** (formats via Koda's SubscriptionTransfer)
...
- **Enhanced player** — storyboard hover previews on the seek bar, double-tap to seek, background playback, speed control, audio-only mode, volume normalization, sleep timer, playback queue (with **related-video radio mode**, **swipe-to-remove**, **restart-surviving persistence**, and **shuffle-all liked videos**), **crossfade between queue items**, configurable playback-notification buttons (like/dislike/radio), **most-played history sort**, **Open in another player** handoff from the quick-actions sheet, **bitrate labels in the quality picker** (Yattee-style), a **synced-lyrics sheet** with live timestamp highlighting (YouTube's own `get_transcript` timed cues, InnerTune-style), live chat polling
...
- **Content filtering** — block channels (persisted), a **"Don't recommend this" dismissal on recommendations** with undo and a management screen, hide watched videos, hide unplayable videos, watched-threshold, shorts shelf toggles, per-surface options to hide counters/likes/comments, dead-code-free quick-actions sheet
