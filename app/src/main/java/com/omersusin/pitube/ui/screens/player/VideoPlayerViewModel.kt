...
class VideoPlayerViewModel @Inject constructor(
                ...
                val streamInfoDeferred = async(PerformanceDispatcher.networkIO) {
                    ...
                    while (info == null && attempt < maxAttempts) {
                        try {
                            ...
                            // Expected when the escalateToSabr path cancels this
                            // deferred — not a real failure, don't burn log noise.
                            throw e
                ...
                kotlinx.coroutines.withTimeout(playbackLoadTimeoutMs) {
                    ...
                    if (streamInfo != null) {
                        ...
                        val captionStreams = innerTubeResult?.playerResponse
                            ?.let { CaptionTrackResolver.resolve(it, preferredCaptionTarget()) }.orEmpty()
    ...
    private suspend fun prepareLiveStreamFromInnerTube(
        ...
        val liveCaptionStreams = StreamProcessor.processSubtitleStreams(
            CaptionTrackResolver.resolve(result.playerResponse, preferredCaptionTarget())
        )
                    ...
                    translationJob = viewModelScope.launch {
                        ...
                        val translated = com.omersusin.pitube.data.lyrics.LyricsTranslationRepository
                            .translate(
                                ...
                                targetLangOverride = prefsL.translationTargetLanguage
                                    .first()
                                    .ifBlank { prefsL.lyricsTranslationTargetLang.first() },
    ...
    /**
     * Caption translation target from settings, or null when disabled.
     * Subtitles use YouTube's native `tlang` auto-translation (no API key,
     * perfectly synced), so they never touch the translation engines.
     */
    private suspend fun preferredCaptionTarget(): String? {
        if (!playerPreferences.translateCaptions.first()) return null
        return playerPreferences.translationTargetLanguage.first().takeIf { it.isNotBlank() }
    }
