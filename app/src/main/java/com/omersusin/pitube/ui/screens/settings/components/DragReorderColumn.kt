 ...
 * Designed for short settings lists, not hundreds of items.
 */
@Composable
fun <T> DragReorderColumn(
    ...
    var draggingKey by remember { androidx.compose.runtime.mutableStateOf<String?>(null) }
        ...
        items.forEachIndexed { index, item ->
            ...
            Row(
                modifier = Modifier
                    ...
                    .pointerInput(latestItems.size, k) {
                        detectDragGesturesAfterLongPress(
                            ...
                            onDrag = { change, dragAmount ->
                                ...
                                var from = latestItems.indexOfFirst { itemKey(it) == k }
                                if (from < 0) return@detectDragGesturesAfterLongPress
                                while (dragOffsetY > h / 2f && from < latestItems.lastIndex) {
                                    onMove(from, from + 1)
                                    from++
                                    dragOffsetY -= h
                                }
                ...
                Column(modifier = Modifier.weight(1f)) { itemContent(item) }
                Icon(
