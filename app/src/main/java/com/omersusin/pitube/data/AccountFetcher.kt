...
object AccountFetcher {
    ...
    fun getCached(context: Context): AccountInfo? {
        ...
        return AccountInfo(name, p.getString("url", null))
    }

    fun cache(context: Context, info: AccountInfo) {
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit().putString("name", info.name).putString("url", info.avatarUrl).apply()
    }
