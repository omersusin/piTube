...
class StatsRepository(private val context: Context) {
    private val file = File(context.filesDir, "play_history.json"); private val gson = Gson(); private val MAX = 5000
    suspend fun addPlayEvent(video: VideoItem, durationMs: Long) = withContext(Dispatchers.IO) {
        try {
            val h = loadHistory().toMutableList(); val last = h.firstOrNull()
    ...
    suspend fun getGlobalStats(): GlobalStats = withContext(Dispatchers.Default) {
        val h = loadHistory(); if (h.isEmpty()) return@withContext GlobalStats()
        val vs = h.groupBy { it.videoId }.map { (id, es) -> val f = es.first(); VideoStats(id, f.title, f.channelName, f.thumbnailUrl, es.size, es.sumOf { it.duration }) }.sortedByDescending { it.playCount }
        val cs = h.groupBy { it.channelName }.map { (n, es) -> ChannelStats(n, es.size, es.distinctBy { it.videoId }.size) }.sortedByDescending { it.playCount }
        val days = h.map { day(it.timestamp) }.toSortedSet(); var longest = 0; var run = 0; var prev: Long? = null
        for (d in days) { run = if (prev != null && d == prev + 1) run + 1 else 1; if (run > longest) longest = run; prev = d }
        val today = day(System.currentTimeMillis()); var cur = 0; var c = if (today in days) today else today - 1
        while (c in days) { cur++; c-- }
        GlobalStats(h.size, h.sumOf { it.duration / 1000 }, vs.take(10), cs.take(10), cs.size, vs.size, cur, longest)
    }
    fun loadHistory(): List<PlayHistoryEntry> { if (!file.exists()) return emptyList(); return try { gson.fromJson(file.readText(), object : TypeToken<List<PlayHistoryEntry>>() {}.type) ?: emptyList() } catch (e: Exception) { emptyList() } }
    private fun day(ts: Long): Long { val c = Calendar.getInstance(); c.timeInMillis = ts; c.set(Calendar.HOUR_OF_DAY,0); c.set(Calendar.MINUTE,0); c.set(Calendar.SECOND,0); c.set(Calendar.MILLISECOND,0); return c.timeInMillis / 86_400_000L }
}
