...
object KodaAuth {
    ...
    fun refreshFromResponse(context: Context, resp: okhttp3.Response) {
        ...
        if (merged != existing) AuthManager.saveRawCookies(context, merged)
    }

    fun authHeader(cookieString: String, origin: String = "https://www.youtube.com"): String? {
        val sapisid = getSapisid(cookieString) ?: return null
        val ts = System.currentTimeMillis() / 1000
        val hash = MessageDigest.getInstance("SHA-1").digest("$ts $sapisid $origin".toByteArray()).joinToString("") { "%02x".format(it) }
